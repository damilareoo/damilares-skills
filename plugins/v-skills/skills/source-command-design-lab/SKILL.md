---
name: "source-command-design-lab"
...
# source-command-design-lab

Use this skill when the user asks to run the migrated source command `design-lab`.
...
**Never leave `.Codex-design/` or `__design_lab` routes behind.** If the user says "cancel", "abort", "stop", or "nevermind" at any point, confirm and then delete all temporary artifacts.
...
After the interview, create a structured Design Brief as JSON and save to `.Codex-design/design-brief.json`:
...
Create all files under `.Codex-design/`:
...
**Reliability Strategy:** To avoid import path issues across different project configurations, create the FeedbackOverlay **directly in the route directory** (e.g., `app/design-lab/FeedbackOverlay.tsx`), NOT in `.Codex-design/`. This ensures a simple relative import (`./FeedbackOverlay`) always works.
...
**Why this approach:**
- `.Codex-design/` paths can fail due to bundler configurations
...
**Next.js App Router:**
Create `app/__design_lab/page.tsx` that imports from `.Codex-design/lab/`

**Next.js Pages Router:**
Create `pages/__design_lab.tsx` that imports from `.Codex-design/lab/`
...
1. **Output the lab location and URL:**
   ...
   I've generated 5 design variants in `.Codex-design/lab/`
...
1. Create `.Codex-design/preview/` directory:
   ```
   .Codex-design/preview/
...
2. **If confirmed, clean up immediately:**
   - Delete `.Codex-design/` directory entirely
...
- Remove `.Codex-design/` directory entirely
...
**Safety rules:**
- ONLY delete files inside `.Codex-design/`
...
6. Plugin creates: `.Codex-design/lab/` with 5 variants
